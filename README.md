...
**Be easy to use.** Sibyl keeps similar names to R functions (`forecast`, `summarize`, `fitARIMA`) and implements similar algorithms. Statisticians familiar with R should feel right at home. `import Sibyl` gives you a notebook-friendly facade that throws on failure rather than returning `Either`, just like R. For production pipelines, import individual modules like `Sibyl.Safe.TimeSeries` or `Sibyl.Models.ARIMA` directly for full error handling.
